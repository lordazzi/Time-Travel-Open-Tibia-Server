function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	itemweight = itemweight + getItemWeightById(2148, 50);
	itemweight = itemweight + getItemWeightById(2148, 50);
	itemweight = itemweight + getItemWeightById(2148, 50);
	itemweight = itemweight + getItemWeightById(2148, 50);
	...
	if (getPlayerStorageValue(cid, actionid) < 1) then
		...
		if (itemweight <= getPlayerFreeCap(cid)) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You have found a full pirate backpack, it weights " .. itemweight .. "oz, you have not cap.")
		end
	else
